...
export const TradingProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const fetchHistoricalDataHandler = useCallback(async (request: HistoricalDataRequest) => {
    ...
    try {
      const response = await getHistoricalData(selectedBroker, sessionToken, request);
      setHistoricalData(response);

      if (!response.success) {
        setHistoricalDataError(response.errorMessage || 'Failed to fetch historical data');
      }
    } catch (error) {
